...
**G27 POLICY — BATCH BY AXIS, VERIFY AS A BUCKET (owner, 2026-09-15: *"Do we need to check after every somewhat minor change? I think we can work to test multiple things so we can incorporate complete multiple changes, especially when they don't hit the same mechanics"* — CORRECT, and this is now the standing rule for every dispatch):**
...
**RULES THAT FOLLOW (binding on every brief from now on):**
