...
[status: REDESIGN IN FLIGHT 2026-09-14 — the L1+L2 attempt was SHELVED (git stash, labelled "W7b L1+L2 SHELVED"): the real-loop A/B measured it NARROWING divergence (baseline x2.49 -> x1.22 with the change), because L1 diluted the weapon pool GREED_DAMAGE wants and L2 made a card uniformly stronger. DO NOT LAND L1+L2. The redesign is owner-directed and lives in docs/briefs/W7B_DRAFT_DIVERGENCE.md: a RARITY-LADDERED draft (COMMON flat / RARE percent / MYTHIC chase; fixed and percent COEXIST, never a conversion), the chase cards Second Wind (revive 50% HP, ~1/10 runs) and Storm Shards (chip damage on XP pickup, ~1/10 runs) as the owner's priority, plus Whetstone/Scholar's Stone/Gilded Palm/Crimson Edge (rare percent), Full Hand (+1 offer), the L3 dead-Split-Shot fix, and Fortune extended to the whole ladder. THE SIM IS 2x OFF THE REAL BASELINE (x1.36 vs x2.49) so the acceptance gate is a PAIRED-SEED real-loop A/B, never the sim. Dispatched manually by the orchestrator while the goal-pilot is paused.]
