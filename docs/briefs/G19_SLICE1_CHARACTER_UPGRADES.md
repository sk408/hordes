...
## DISPATCH-TIME ADDENDUM (goal pilot `subagent:spawnfa`, 2026-09-16 00:50 UTC)

Anchors RE-RESOLVED on the tree you are receiving, `a52e84f` (the anchor block above was written at
`10b30f9`; one commit landed in between — V1d/V1e escape + the G16 skip-bar respec part 2 — none of it
touches your files). Confirmed verbatim on `a52e84f`:
`src/meta.js` :419 SHOP_UPGRADES, :592 upgradeCost, :601 buyUpgrade, :1062 `CHARACTER_UPGRADES = []` (still
EMPTY), :1063 CHARACTER_UPGRADE_BY_ID, :1068 CHARACTERS, :1108 applyCharacter;
`src/save.js` :43 `PROFILE_VERSION = 8`; `src/main.js` :4544 showShop, :4803 the character preview seam,
:5096 the run-start seam. If any of these has moved by the time you start, re-resolve by grep and SAY SO.
Inherited suite state at dispatch: `TREE @ a52e84f | dirty=1` (one PNG regenerated by a verifier, not
code), `SUITE greenfiles=98 redfiles=0`, REDLIST empty — so you are starting from green and any red is
yours.
