...
**Not covered by 1.8, by design:** the galleries (TROPHIES / BESTIARY / APEX) keep Left/Right *ring-stepping*; the shop and the manual keep Left/Right *paging*; and the DRAFT / EVOLVE / INTERMISSION / DEAD / CHEST screens keep their documented number-key and single-letter contracts (they already work keyboard-only). DIM cards (a pager end, a locked row) are **skipped** by the cursor, so Enter can never fire a card the pointer would refuse.
