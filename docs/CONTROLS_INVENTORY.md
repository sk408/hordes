...
**NOT covered by 1.8, by design:** the galleries (TROPHIES / BESTIARY / APEX) keep Left/Right *ring-stepping*; the shop and the manual keep Left/Right *paging*; the CHEST card keeps its single `GOT IT` key (one card, nothing to move between). DIM cards (a pager end, a locked row) are **skipped** by the cursor, so Enter can never fire a card the pointer would refuse.
