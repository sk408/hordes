...
### 2a. Touch-layer DOM buttons (`#touch`, shown on coarse-pointer/touch devices; `index.html:201-216`, handlers `src/main.js:2521-2534`)
...
| 2.4 | **STATS** button (`tc-stats`) | Opens the FIELD REPORT (loadout/gear/stats overlay); pauses the sim | Touch layer, in-run only | Tap DOM button `data-act="stats"` | `index.html:206` (id added WAVE-22); action `src/main.js:2319-2323`; opener `src/main.js:2227` | **YES** — WAVE-22 coachmark `stats` (t>22s): spotlights `tc-stats` via `statsTarget()` (falls back to the loadout canvas region on keyboard-only devices), names S / I |
| 2.5 | **FROST** skill button (`tc-q`) | Fires Frost Nova (AoE freeze/slow, 30 mana); badge shows cooldown or RDY/LOW | Touch layer, in-run only | Tap DOM button `data-act="q"` (multi-touch safe) | `index.html:210`; action `src/main.js:2340` | **YES** — stage-2 coachmark `skills` step 1 (t>16s, targets `tc-q`) |
| 2.6 | **OVER** skill button (`tc-w`) | Fires Overcharge (attack-rate buff, 25 mana); badge shows cooldown/RDY | Touch layer, in-run only | Tap DOM button `data-act="w"` | `index.html:211`; action `src/main.js:2341` | **YES** — WAVE-22: `skills` is now a TWO-step coach; step 2 spotlights `tc-w` itself and names the W-in-AUTO / E-always subtlety |
...
| 2.9 | **Settings cog** (`tc-cog`) | Opens the in-run SETTINGS pause screen (zoom, END RUN, audio, replay tour) | Touch layer, in-run only; pinned top-right | Tap DOM button `data-act="settings"` | `index.html:215` (CSS cog `index.html:151-186`); action `src/main.js:2325-2329`; opener `src/main.js:2300-2304` | **YES** — stage-2 coachmark `cog` (`src/main.js:1942-1945`) |
...
| 2.18 | **S / I** — Field Report | Opens the stats overlay (S only in AUTO — S is "down" in MANUAL; I always) | In-run | Press `S` (AUTO) / `I` (always) | `src/main.js:2417,2427` | **YES** — named in the `stats` coachmark |
...
| 2.30 | **Overlay key-hint footer** | Static line under every overlay: "1/2/3 or tap a card · TAB focus · G stance · Q frost / E overcharge · H/N potions · M pilot (WASD+arrows manual) · +/- zoom · S or I stats · or the on-screen buttons" | Visible whenever the overlay is up (menus/drafts/death) | — (static text, not a control) | `index.html:221` | **VERIFIED** (WAVE-22) — every key on the line checked against the handlers (`src/main.js:2416-2437`): all accurate. Non-goal as a tour target (static cheat line) |
...
| # | Name | What it does | When it appears | How activated | Source | Tour? |
|---|------|--------------|-----------------|---------------|--------|-------|
| 4.16 | **WEAPONS / ITEMS / SYNERGIES / THE NUMBERS cards** | Read-only loadout detail: weapons + evolutions, equipped items + affixes, active synergies, rampage/core stats | FIELD REPORT overlay (S/I key or STATS button) | Tap/click (informational) | `src/main.js:2252,2264,2272,2276` | **DOC** — the screen is introduced by the `stats` coachmark (2.4/2.18); the informational cards are non-goals |
...
3. **STATS / FIELD REPORT** — coachmark `stats` (t>22s): button (or loadout region fallback) + S/I named.
