...
const EXPECTED_TROPHIES = [
  ...
  'ARCADE_PASS', 'FULL_BUILD', 'LOCKED',
];
const EXPECTED_PORTRAITS = ['KNIGHT', 'WITCH', 'ROGUE', 'PALADIN'];
// meta.js SHOP_UPGRADES ids at the time of authoring: 14 stat/slot rows
// (dmg..luck, slots, arcade) + 7 priced weapon unlock rows + 3 elite unlock
// rows = 24 rows, PLUS the two STARTER_WEAPONS (VOLLEY, BOOMERANG) which have
// no shop row but do have art (the draft pool / roster UI uses the same
// icons), PLUS the generic fallback = 27 keys.
const EXPECTED_SHOP = [
  'dmg', 'hp', 'potions', 'regen', 'xp', 'crit', 'critdmg', 'greed', 'alchemy',
  'scav', 'artifact', 'luck', 'slots', 'arcade',
