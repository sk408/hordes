...
for (let i = 0; i < frames; i++) {
  ...
  // Wave probes: typed enemies + the wave-5 trio on the FOES census line.
  {
    ...
    const pos = t.match(/POS (-?\d+),(-?\d+) MV (-?\d+.\d+),(-?\d+.\d+) @(\d+)/);
    if (!pos && stillFrames > 20) console.log('MVMISS ' + t.split('\n').slice(-1));
    const foes = t.match(/FOES (C:\d+ .*?)\n/);
    if (pos && foes) {
      const x = +pos[1], y = +pos[2], mvx = +pos[3], mvy = +pos[4], pspd = +pos[5];
      const total = (foes[1].match(/(\d+)/g) || []).reduce((s2, v) => s2 + +v, 0);
      const moved = lastPos === null ? true : (Math.abs(x - lastPos[0]) + Math.abs(y - lastPos[1])) >= 1;
      if (!moved && total > 10) {
        stillFrames++;
        if (stillFrames === 15) {
          console.log('STALLCTX i=' + i + ' mv=' + mvx + ',' + mvy + ' spd=' + pspd +
            ' pos=' + x + ',' + y + ' | ' + t.split('\n').slice(-4).join(' / '));
        }
