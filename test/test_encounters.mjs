...
S.check('v4 -> v5: migration [4], an empty namespace appears, no entry invented', () => {
  ...
  assert.deepEqual(res.migrations, [4, 5, 6, 7, 8, 9], 'v4->v5 ran, then v5->v6 (banner ledger), then the v6->v7 run-purse step, the v7->v8 apex step, the v8->v9 lastPlayed step, then this build\'s v9->v10 milestoneChest step');
...
S.check('v4 with garbage encounters: entry-safe either way', () => {
  ...
  assert.deepEqual(str.migrations, [4, 5, 6, 7, 8, 9], 'the migration ran (through the v9→v10 milestoneChest step)');
  ...
  assert.deepEqual(bad.migrations, [4, 5, 6, 7, 8, 9], 'the migration ran (through the v9→v10 milestoneChest step)');
