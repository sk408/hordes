// HORDES — tools/verify_g8_skills.mjs: G8 STEP 4 in a REAL browser, on a PHONE.
// Same bar as verify_g8_rules.mjs (the build plan's rule: anything a player
...
// 390x844 @3x, seeds drafts until a SKILL card (Regrowth / Focus / Thick Skin
// — family weight 3 x SKILL_CARD_WEIGHT, the rarest family in the pool) reaches
// the offer, reads the rendered card geometry out of the DOM (is the desc
// clipped? is every card inside the viewport?) and samples the captured PNG
// inside the skill card.
//
...
//
// Run: node tools/verify_g8_skills.mjs
...
const out = await withPage({ w: 390, h: 844, dpr: 3,
  ...
  async (p) => {
    ...
    const res = await p.evaluate(`(async () => {
      ...
      try {
        for (let i = 1; i <= 400 && !found; i++) {
          ...
          const hit = els.find(el => (el.textContent || '').includes('SKILL - '));
    ...
    const shot = await p.shot('g8-step4-skill-draft-phone');
    const target = res.cards.find(c => c.text.includes('SKILL - ')) || res.cards[0];
    // readShot takes a LABEL -> [x, y] map in CSS px (it scales by the captured dpr).
    const px = await p.readShot(shot, {
      skillCardText: [Math.round(target.rect.x + target.rect.w / 2), Math.round(target.rect.y + 40)],
      skillCardBody: [Math.round(target.rect.x + target.rect.w / 2), Math.round(target.rect.y + target.rect.h / 2)],
    });
    return { ...res, shot, sample: px };
  });

console.log(JSON.stringify(out, null, 2));
const skill = (out.cards || []).find(c => c.text.includes('SKILL - '));
const problems = [];
if (out.mode !== 'draft') problems.push('mode is ' + out.mode + ', not draft');
if (!out.found) problems.push('no SKILL card reached the pool in ' + out.tries + ' drafts');
