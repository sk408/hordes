// HORDES — tools/verify_g8_rules.mjs: G8 STEP 3 in a REAL browser, on a PHONE.
// The build plan's bar: anything a player looks at is verified by a real-browser
// screenshot on a phone viewport, and a code claim is not evidence. The two new
// cards are the first draft cards whose TEXT wraps to two lines in the pool, so
// this drives the REAL src/main.js openDraft() at 390x844 @3x, forces a rule card
// into the offer, reads the rendered card geometry out of the DOM (is the desc
// clipped?) and samples the captured PNG inside the rule card.
//
// Run: node tools/verify_g8_rules.mjs
...
const out = await withPage({ w: 390, h: 844, dpr: 3,
  ...
  async (p) => {
    ...
    const res = await p.evaluate(`(async () => {
      ...
      try {
        for (let i = 1; i <= 400 && !found; i++) {
          ...
          const hit = els.find(el => (el.textContent || '').includes('RUN RULE'));
    ...
    const shot = await p.shot('g8-step3-draft-rule-phone');
    const target = res.cards.find(c => c.text.includes('RUN RULE')) || res.cards[0];
    // readShot takes a LABEL -> [x, y] map in CSS px (it scales by the captured dpr).
    const px = await p.readShot(shot, {
      ruleCardText: [Math.round(target.rect.x + target.rect.w / 2), Math.round(target.rect.y + 40)],
    });
    return { ...res, shot, sample: px };
  });

console.log(JSON.stringify(out, null, 2));
const rule = (out.cards || []).find(c => c.text.includes('RUN RULE'));
const problems = [];
if (out.mode !== 'draft') problems.push('mode is ' + out.mode + ', not draft');
if (!out.found) problems.push('no RUN RULE card reached the pool in ' + out.tries + ' drafts');
...
console.log(problems.length ? 'VERIFY G8 STEP 3: FAIL - ' + problems.join('; ')
  : 'VERIFY G8 STEP 3: PASS - phone draft shows the rule card unclipped');
